...
const logo: string = `___________________________________
|#######====================#######|
...
const login: {username:string; password:string} = await inquirer.prompt([
...
if (!(login.username === 'azhar' && login.password === '1234')) {
    ...
    while(_continue) {
        const response: { what: string } = await inquirer.prompt(
            [{
                name: 'what',
                type: "list",
        ...
        switch (response.what) {
            ...
            case 'tm':
                ...
                const respTM: {targetAccount:string; amount:string} = await inquirer.prompt([
                    {
                        name: 'targetAccount',
                        type: 'input',
                ...
                break;
            case 'pub':
                const respPub: {utility:string} = await inquirer.prompt([
                    {
                        type:'input',
                        name:'utility',
                ...
                const respPB:{proceed: boolean} = await inquirer.prompt([{
                    name:'proceed',
                    type:'confirm',
                ...
                break;
            case 'da':
                const respDA :{amount:string} = await inquirer.prompt([{
                    name:'amount',
                    type:'list',
                    message:'Amount',
                ...
                {
                    const respDACustom: {amount:number} = await inquirer.prompt([{
                        name : 'amount',
                        type: "number",
